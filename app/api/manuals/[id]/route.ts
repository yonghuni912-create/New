  ...
  try {
    const manual = await prisma.menuManual.findUnique({
      ...
      include: {
        ...
        costVersions: { include: { template: true, costLines: true }, orderBy: { createdAt: 'desc' } }
      }
    });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.koreanName) updateData.nameKo = body.koreanName;
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  try {
    await prisma.menuManual.update({ where: { id }, data: { isActive: false, isDeleted: true } });
